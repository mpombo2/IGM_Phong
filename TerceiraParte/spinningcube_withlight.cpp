...
unsigned int texture = 0; // Texture to paste 
...
// Vertex Buffer Object (for vertex coordinates)
  ...
  GLfloat vertex_normals[sizeof(float)* (36+12)*3];     //(36 vertices + 12 vertices) * 3 coordenadas/vertice
...
unsigned int loadTexture()
{
    ...
    unsigned char *data = stbi_load("texture.jpg", &width, &height, &nrComponents, 0);
   // unsigned char *data = stbi_load("watchmen_smiley.png", &width, &height, &nrComponents, 0);
    if (data)
    {
        GLenum format;
        if (nrComponents == 1)
        ...
        // Axuste se nos pasamos do rango (0.0,0.0) : (1.0,1.0)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);

        // Opcións de filtrado de texturas para o mapeo texel - texCoord
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

        //Xeramos a textura a partir da imaxe cargada
        glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, format, GL_UNSIGNED_BYTE, data);
        // Xeramos os mipmaps para a textura
